...
const flash = require("express-flash") // INSTALL FLASH
...
app.post("/addProject", upload.single("image"), addTheProject)
...
async function addTheProject (req, res) {
    ...
    const query = `INSERT INTO blogs(title,content,image,user_id,"createdAt","updatedAt") 
	VALUES('${title}', '${content}','${image}','${userId}',now(),now())`
    const data = await sequelize.query(query, {type : QueryTypes.INSERT})
...
function addDetailProject (req, res) {
    ...
    const { title, content, endDate, startDate, id, name } = req.body;
    data [id] = {
        title, content, endDate, startDate, image, name
    }
    res.redirect("myProject")   
}
...
async function detailProjectView (req, res) {
    ...
    const { id } = req.params;
    // const query = "SELECT * FROM blogs, users WHERE id=" + id
    // const query = `SELECT * FROM blogs WHERE id=${id}` // Cara kedua
    const query = `SELECT public.blogs.id, public.blogs.title, public.blogs.content, public.blogs.image, public.blogs."createdAt", public.users.name FROM public.users INNER JOIN public.blogs ON public.users.id = public.blogs.user_id;` // Cara kedua
...
async function editProject (req, res) {
    ...
    const { title, content, startDate, endDate, id } = req.body;
    // data [id] = {
    //     title, content, endDate, startDate, id
    // }
...
async function myProject (req, res) {
    ...
    const query = `SELECT public.blogs.id, public.blogs.title, public.blogs.content, public.blogs.image, public.blogs."createdAt", public.users.name FROM public.users INNER JOIN public.blogs ON public.users.id = public.blogs.user_id;`;
    const data = await sequelize.query(query, {type: QueryTypes.SELECT});


    // CARA MENGGUNAKAN ORM 
